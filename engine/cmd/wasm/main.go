...
func runStatWeights(_ js.Value, args []js.Value) any {
	if len(args) != 1 {
		return jsError("wowfSimStatWeights expects a Uint8Array")
	}
	...
	out, err := proto.Marshal(sim.RunStatWeights(req))
	if err != nil {
		return jsError(err.Error())
	}
...
func main() {
	...
	js.Global().Set("wowfSimStatWeights", js.FuncOf(runStatWeights))
